...
)
{
    ...
    for(i in 1:mc.cores) {
        parallel::mcparallel({psnice(value=nice);
                  mbart(x.train=x.train, y.train=y.train, x.test=x.test,
                        sparse=sparse,  theta=0, omega=1,
    ...
    if(mc.cores==1 | attr(post, 'class')!='mbart') return(post)
        ...
        for(j in (1:post$C)+1) post$treedraws[[j]] <- sub(old.text,
        ...
        for(i in 2:mc.cores) {
            ...
            for(j in (1:post$C)+1) post$treedraws[[j]] <- paste0(post$treedraws[[j]],
                                           substr(post.list[[i]]$treedraws[[j]], old.stop+2,
                                                  nchar(post.list[[i]]$treedraws[[j]])))
